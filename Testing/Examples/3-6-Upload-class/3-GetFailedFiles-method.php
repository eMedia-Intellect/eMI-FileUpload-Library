...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['formSubmission']))
{
	...
	{
		if (!empty($Upload->GetFailedFiles()))
		{
			foreach ($Upload->GetFailedFiles() as $file)
			{
				$Output .= $Indentation . "<p>$file was unsuccessfully uploaded.</p>\n";
			}
		}
		else
		{
			$Output .= $Indentation . "<p>No files were unsuccessfully uploaded!</p>\n";
		}
	}
	else
	{
