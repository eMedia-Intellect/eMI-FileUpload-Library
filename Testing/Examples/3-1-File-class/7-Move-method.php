...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['formSubmission']))
{
	...
	if ($Upload->GetHasUploaded())
	{
		if ($Upload->GetFiles() !== null)
		{
			...
			{
				if ($file->Move('/var/tmp/php/Uploads'))
				{
					$Output .= $Indentation . '<p>' . $file->GetName() . " was moved.</p>\n";
				}
				else
				{
					$Output .= $Indentation . '<p>' . $file->GetName() . " was not moved.</p>\n";
				}
			}
		}
		else
		{
			$Output .= $Indentation . "<p>No files were successfully uploaded!</p>\n";
		}
	}
	else
	{
